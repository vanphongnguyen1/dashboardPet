...
const PendingOrder = ({ data }) => {
  ...
  useEffect(() => {
    if (
      ...
      const [dataPendding] = sectionData(
        data,
      ...
      if (!Array.isArray(dataPendding)) {
        setDataOrderShow([dataPendding].sort((a, b) => -a.id + b.id))
      } else {
        setDataOrderShow(dataPendding.sort((a, b) => -a.id + b.id))
      }
    }
  ...
  const newData = dataOrderShow.slice(0, 5)

  return (
    <>
      <div className="pending-order">
        <h2 className="pending-order__heading">Pending Orders</h2>

        <ul className="pending-order__list">
          {dataOrderShow &&
            newData.map((item) => {
              return <ItemOrderPending data={item} key={item.id} />
            })}
        </ul>
