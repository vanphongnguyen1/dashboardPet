...
export const fetchUsers = createAsyncThunk('users/fetchUsers', async () => {
  return customAxiosApi.get(API_NAME.USERS).then((response) => {
...
export const fetchUser = createAsyncThunk('users/fetchUser', async (id) => {
  return customAxiosApi.get(`${API_NAME.USERS}/${id}`).then((response) => {
    const { data } = response.data
    return data
  })
})
...
export const { defaultUsers, defaultState } = userSlice.actions
